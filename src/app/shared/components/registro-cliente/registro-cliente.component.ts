...
generos = [
  ...
  mostrarDialogo(documento?: string): void {
    ...
    if (documento) {
      this.datosPersonaNatural.documento = documento;
      this.datosEmpresa.documento = documento;
    } else if (this.documentoPrevio) {
      this.datosPersonaNatural.documento = this.documentoPrevio;
      this.datosEmpresa.documento = this.documentoPrevio;
    }
  ...
  resetearFormulario(): void {
    ...
    this.datosPersonaNatural = {
      documento: this.documentoPrevio || '',
      nombres: '',
      apellidos: '',
  ...
  if (this.isPersonaNatural) {
    return {
      ...
      // ✅ Simplificar el género - ya viene con el valor correcto
    ...
    error: (error) => {
      ...
      if (error.status === 400) {
        if (error.error && error.error.includes('ya existe')) {
          ...
          // ✅ Emitir en el formato correcto para cliente existente
          this.clienteRegistrado.emit({
            documento: cliente.documento,
            cliente: null  // null indica que ya existe
          });
          
          this.documentoPrevio = '';
          this.visible = false;
          this.resetearFormulario();
          return;
        }
        ...
        // ✅ También emitir para clientes duplicados
        this.clienteRegistrado.emit({
          documento: cliente.documento,
          cliente: null
        });
